...
- **assay runs one thing, and only one.** When it offers to turn a rule into a hook, it first tests that hook: it writes the hook's own files into a temporary folder, runs the command there against an example it must catch and a near one it must let through, and throws the hook away unless it gets both right. That test runs before you are asked to approve anything, it runs in the temporary folder rather than in your project, and it is capped at ten seconds. Nothing else assay does executes anything — every other check reads files.
