...
description: >-
  ...
  skills, or subagents. Most of the scoring is a deterministic Node script; the
  model judges only two factors.
  Offers to rewrite weak rules and to park placement candidates for promotion.
  English-only scoring. Never reviews code, PRs, or non-Claude config like eslint.
when_to_use: >-
...
If `ruleCount` is 0, tell the user no rules were found and stop.
...
placement candidates. Present it as-is, then add at most 3 sentences of your
...
- `Promote [N] candidates now` — only if placement candidates exist. Description:
  "Build each one with its companion plugin — hookify, skill-creator,
  plugin-dev — installing any missing companion on the spot."
