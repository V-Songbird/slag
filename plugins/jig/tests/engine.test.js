...
test("the per-kind allowlist decides where a kind may land inside the root", () => {
  ...
  assert.match(String(engine.targetProblem(root, "write-settings", "src/index.js")), /retired Claude Code change kind/);
...
test("write-settings remains refused even when a historical permission probe is green", () => {
  ...
  try {
    const stillRefused = engine.planFromDraft({ changes: settingsDraft }, green);
    assert.match(stillRefused.problems.join(" "), /Historical Claude Code permission probes/);
...
// ---------------------------------------------------------------------------
// The native Codex harness brief; checks-rule is an alias for agents-region.

test("--checks-rule writes the same brief as the region, item tier, and reverts", () => {
  const root = tmpProject({});
  const plan = engine.cmdPlan(root, {
    _: [], change: [], select: REGION_SELECT, "no-ci": true,
    "checks-rule": true, "agents-region": true, provenance: "elicited",
  });
  const rule = plan.changes.find((c) => c.path === "AGENTS.md");
  assert.ok(rule, "no rule was proposed");
  const artifact = JSON.parse(fs.readFileSync(path.join(root, ".jig", "plan.json"), "utf-8"))
    .artifacts.find((a) => a.id === rule.id);
  assert.equal(artifact.tier, "item");

  engine.cmdApply(root, {
    _: [], change: plan.changes.map((c) => c.id), path: plan.changes.map((c) => c.path),
  });
  const text = fs.readFileSync(path.join(root, "AGENTS.md"), "utf-8");
  const region = fs.readFileSync(path.join(root, "AGENTS.md"), "utf-8");
  // Same body in both hosts, from the same selection: two channels, one claim.
  const body = "jig guards this repository." + text.split("jig guards this repository.")[1].split("<!--")[0];
  assert.ok(region.includes(body.trimEnd()), "the rule and the region say different things");
  assert.match(text, /silent-catch, test-file-deletion/);
  assert.equal(plan.changes.filter((c) => c.kind === "write-agents-region").length, 1, "aliases must share one reviewed write");

  engine.cmdRevert(root, { _: [], change: [], all: true });
  assert.equal(fs.existsSync(path.join(root, "AGENTS.md")), false);
});

function checksRuleText(opts) {
  const root = tmpProject({});
  const plan = engine.cmdPlan(root, {
    _: [], change: [], select: REGION_SELECT, "checks-rule": true, provenance: "elicited", ...opts,
  });
  engine.cmdApply(root, {
    _: [], change: plan.changes.map((c) => c.id), path: plan.changes.map((c) => c.path),
  });
...
test("the brief names the lanes as they now run", () => {
  const text = checksRuleText({});
  assert.match(text, /--verify --lane ci/);
  assert.match(text, /--selftest/);
  assert.match(text, /staged bytes/);
});

// Always-loaded prose naming a lane the plan did not write is the coverage claim
// SCOPE forbids, on the surface every session reads.
test("the brief claims no CI lane where the plan wrote no workflow", () => {
  const text = checksRuleText({ "no-ci": true });
  assert.doesNotMatch(text, /--verify --lane ci/,
    "the brief tells every session CI runs a lane that --no-ci never wired");
  assert.match(text, /No CI workflow was installed/);
  assert.match(text, /staged bytes/);
});
