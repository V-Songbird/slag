...
- `$jig` routes plain-language status and activity requests to the existing
  inventory and review workflows in the same conversation. Direct entries remain.
- Ordinary reports start with coverage or activity, separate session/commit/CI
  evidence, and anything requiring attention. Full and section detail remain
  available; report-only requests do not trigger an action menu or mutation.
- Setup uses three visible stages, reuses answered preferences, and distinguishes
  proposal choices from approval of exact changes. Named approvals, detector
  admission, blocking defaults, hooks, stored data and undo remain unchanged.
- The README starts with everyday use; technical coverage and reversal details
