...
#
# If node is not on PATH in the hook's environment (fnm/nvm/volta), the driver
# is skipped and the commit proceeds without these checks. CI coverage must be
# checked separately. Only check-driver detectors and opted-in commit verify
# entries run here; session-only detectors are not evaluated by this hook.
# See .jig/activation.md for the configured lanes and their limits.
#
...
if ! command -v node >/dev/null 2>&1; then
  echo "jig: node is not on PATH here, so the commit-time checks were skipped. This hook provided no check coverage; inspect CI separately." >&2
