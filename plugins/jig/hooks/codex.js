...
function runEvent(lib, root, event, payload, warn) {
  if (payload.tool_name !== "apply_patch") {
    const out = lib.runEvent(root, event, payload, warn,
      { host: payload.hook_event_name === event ? "codex" : undefined });
  ...
  try {
    ...
    const out = combine(event, views.map((input) => lib.runEvent(root, event,
      { ...payload, tool_name: "Edit", tool_input: input }, warn, { ledgerTool: "apply_patch", host: payload.hook_event_name === event ? "codex" : undefined })));
    if (problems.length) {
