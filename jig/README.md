...
- **Nothing blocks, this release.** Every guard observes and records what it *would* have stopped. You get the evidence first, not the false positives.
- **One command undoes everything.** Every write is journaled with the original bytes. Revert puts your repo back exactly as it was.
...
> [!NOTE]
> The one false alarm is disclosed, not hidden: a download-and-run command quoted inside an `echo` — printed, never executed — fools the session guard that reads commands. That check is labeled a heuristic for exactly this reason, and it only ever records.
...
- This release records; it never blocks. A guard that matches writes a line in a local ledger and the call goes through.
- jig writes only under `.jig/` and `.github/workflows/`. The pre-commit hookup line and the permission rules are printed as proposals for you to apply — jig never touches a file you own.
