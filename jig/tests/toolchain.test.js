...
// Roadmap 255. One package manager can be two programs. `yarn npm audit
// --severity high` is Yarn Berry's command, and on yarn 1.22 it is `error
// Command "npm" not found.` with exit 1 — the very code the audit row calls a
// catch. `pickPackageManager` cannot tell them apart: both write a file called
// `yarn.lock` and it reads the name, never the header. So the entry states the
// marker that rules it out and the tool is refused onto the plan.
test("a byManager entry that cannot run here is refused, never wired", () => {
  const js = edition("javascript-typescript");
  const classic = tmpProject({
    "package.json": "{ \"name\": \"y\", \"private\": true }\n",
    "yarn.lock": "# THIS IS AN AUTOGENERATED FILE. DO NOT EDIT THIS FILE DIRECTLY.\n# yarn lockfile v1\n\n",
  });
  assert.throws(() => toolchain.proposeInstalls(classic, js, ["audit"], "yarn"), (err) => {
    assert.equal(err.expected, true, "a refusal the plan can print, not a crash");
    assert.match(err.message, /audit is not offered under yarn in this project/);
    assert.match(err.message, /Yarn Berry's command/);
    return true;
  });
  // A berry lockfile is the same manager and the same row, and it is offered.
  const berry = tmpProject({
    "package.json": "{ \"name\": \"y\", \"private\": true }\n",
    "yarn.lock": "__metadata:\n  version: 8\n",
  });
  const [item] = toolchain.proposeInstalls(berry, js, ["audit"], "yarn");
  assert.equal(item.command, "yarn npm audit --severity high");
  // And every other manager is untouched by a marker that is not theirs.
  for (const manager of ["npm", "pnpm", "bun"]) {
    const [row] = toolchain.proposeInstalls(classic, js, ["audit"], manager);
