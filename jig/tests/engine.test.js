...
// ---------------------------------------------------------------------------
// The harness brief for the Claude Code session (2.9.0, C7)
...
test("--checks-rule writes the same brief as the region, item tier, and reverts", () => {
  ...
  const rule = plan.changes.find((c) => c.path === ".claude/rules/jig-checks.md");
  ...
  const text = fs.readFileSync(path.join(root, ".claude/rules/jig-checks.md"), "utf-8");
  ...
  engine.cmdRevert(root, { _: [], change: [], all: true });
  assert.equal(fs.existsSync(path.join(root, ".claude/rules/jig-checks.md")), false);
});
...
function checksRuleText(opts) {
  ...
  return fs.readFileSync(path.join(root, ".claude/rules/jig-checks.md"), "utf-8");
}
