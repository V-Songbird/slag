...
const { safeWriteFileSync, logObservation } = require('../hooks/plumb-lib');
...
function tmpDir() {
  const d = fs.mkdtempSync(path.join(os.tmpdir(), 'plumb-safewrite-'));
  dirs.push(d);
  return d;
}
...
describe('safeWriteFileSync: normal writes', () => {
  test('round-trips content to a new file', () => {
    ...
    safeWriteFileSync(target, '{"armed":false}');
    assert.strictEqual(fs.readFileSync(target, 'utf-8'), '{"armed":false}');
...
describe('logObservation: symlinked log path', () => {
  test('skips the append when the log path is a symlink', () => {
    const dir = tmpDir();
    const logFile = path.join(dir, 'plumb-observations.jsonl');
    const origEnv = process.env.PLUMB_LOG;
    process.env.PLUMB_LOG = logFile;
    const origLstat = fs.lstatSync;
    fs.lstatSync = (p, ...rest) =>
      p === logFile ? { isSymbolicLink: () => true, isDirectory: () => false } : origLstat(p, ...rest);
    try {
      logObservation({ kind: 'test' });
      assert.strictEqual(fs.existsSync(logFile), false);
    } finally {
      fs.lstatSync = origLstat;
      if (origEnv === undefined) delete process.env.PLUMB_LOG;
      else process.env.PLUMB_LOG = origEnv;
    }
  });

  test('still appends when the log path is a normal file', () => {
    const dir = tmpDir();
    const logFile = path.join(dir, 'plumb-observations.jsonl');
    const origEnv = process.env.PLUMB_LOG;
    process.env.PLUMB_LOG = logFile;
    try {
      ...
      assert.strictEqual(JSON.parse(lines[0]).kind, 'test');
    } finally {
      if (origEnv === undefined) delete process.env.PLUMB_LOG;
      else process.env.PLUMB_LOG = origEnv;
    }
