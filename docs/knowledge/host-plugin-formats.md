...
| Antigravity CLI | Measured, agy 1.2.8, 2026-09-22: with no project named in the prompt, the model in headless agy gave each run_command the working directory ~/.gemini/antigravity-cli/scratch. Whether agy names that folder to the model as a scratch directory, and whether it is per session, was not checked. | Not checked. Headless agy runs no shell command without a permission rule, as above. |
