...
`<JIG_ROOT>/skills/<skill-name>/`, so the plugin root is two parent directories
...
For example, `node "<JIG_ROOT>/scripts/jig.js" scan` works in PowerShell and a
...
Use `$jig`, `$review` and `$inventory` to invoke these skills. If another plugin
