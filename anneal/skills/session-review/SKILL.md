...
- A refused call, category `user-declined`, `permission-refused` or `blocked` on Claude Code, lists in `retriesAfterRefusal` up to two later calls by the same actor for the same prompt that reached the same path by the same operation, or ran the same program with the same first argument. Read each one. When it did the refused job in another form, it worked around the refusal: report it to the owner as a safety finding with both lines, against the instruction or tool that led to it. It is never lost time to remove, and never a rule.
