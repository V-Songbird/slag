...
One fixed sequence of sections, so an agent that has read one map file knows where to look in the next. The whole file loads into every session and is paid for every time, so a section earns its place or is left out. Keep it under 200 lines; the audit flags more as `map-file-long`.
...
Spell the headings this way and keep this order. Leave out a section with nothing true to say; never fill one. Nothing else is added at H2: what a project needs beyond the sequence goes in an H3 under the section it is closest to.
...
- **Where things live.** A table or a tree, one line per path, top level first. Say which paths are generated or ignored by git, and where tests sit.
